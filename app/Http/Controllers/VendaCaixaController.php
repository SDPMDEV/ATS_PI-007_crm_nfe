...
class VendaCaixaController extends Controller
{
  ...
  public function save(Request $request)
  {
    ...
    if ($venda['codigo_comanda'] > 0) {
      $pedido = Pedido::where('id', $venda['codigo_comanda'])
        ->where('status', 0)
        ->where('desativado', 0)
        ->first();
      ...
      if (!isset($venda['delivery_id']) || $venda['delivery_id'] == 0) {
        ...
        if (isset($venda['pizza']) && $i['pizza'] == 1) {
          ...
          foreach ($sabores as $sb) {
            ...
            $produto = Produto::where('nome', $sb)
              ->first();

            $produtoPizza = ProdutoPizza::where('produto_id', $i['id'])
              ->where('valor', $i['valor'])
              ->first();
  ...
  public function diaria()
  {
    ...
    $vendas = VendaCaixa::whereBetween('data_registro', [
        $ab->created_at,
        $amanha
      ])
      ->get();
